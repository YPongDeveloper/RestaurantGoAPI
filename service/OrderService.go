...
func GetOrderById(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, response.SuccessResponse(orderResponse))
}
...
func GetOrderDateHistory(c echo.Context) error {
	...
	if dateStr == "" {
		return c.JSON(http.StatusBadRequest, response.ErrorResponse("Invalid date"))
	}
...
func CreateOrder(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, response.ErrorResponse("Failed to create customer"))
	}
	tableId, err := repository.FindAvailableTable(createOrder.Number)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, response.ErrorResponse("Failed to find available table"))
	}
	employeeId, err := repository.FindEmployeeId()
	if err != nil {
		return c.JSON(http.StatusInternalServerError, response.ErrorResponse("Failed to find employee"))
	}
	orderId, err := repository.CreateOrder(tableId, customerId, employeeId)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, response.ErrorResponse("Failed to create order"))
	}
